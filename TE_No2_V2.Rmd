...
aq_final <- aq_keep  %>% group_by(Date) %>% summarise(mean_no2=mean(value,na.rm = T),
                                                      median_no2=median(value,na.rm = T))

ggplot(data=aq_final)+
  geom_point(mapping=aes(x=Date,y=median_no2),color="green")+
  geom_point(mapping=aes(x=Date,y=median_no2),color="blue")


aq_final <- aq_final %>% select(-mean_no2)
...
ggplot(data=data)+
  geom_point(mapping=aes(x=Date,median_no2,color=Conf_level))+ scale_fill_hue(l=40, c=35)



ggplot(data=data)+
  geom_point(mapping=aes(y=median_no2,x=T_min,color=month))+facet_grid(month~.)
```
...
ggplot(data=data)+
  geom_histogram(mapping=aes(x=median_no2))+facet_grid(weekday~.)

ggplot(data=data)+
  geom_histogram(mapping=aes(x=median_no2))+facet_grid(month~.)
...
data$no2_lag1 <- lag(data$median_no2,1)

data$no2_lag7 <- lag(data$median_no2,7)
  
data$no2_lag28 <- lag(data$median_no2,28)

data$no2_lag1y <- lag(data$median_no2,7*52)

data <- data %>% filter(!is.na(no2_lag1y))

 ggplot(data=data)+
  geom_point(mapping=aes(y=median_no2,no2_lag7,color=weekday))
ggplot(data=data)+
  geom_point(mapping=aes(y=median_no2-no2_lag1,x=Date,color=month))

ggplot(data=data)+
  geom_point(mapping=aes(y=median_no2-no2_lag1,x=Date,color=weekday))
```
...
for (i in seq(1,nrow(tune_grid))){
   rf_gridsearch[[i]] <-  randomForest(median_no2 ~ ., data=DF_1 %>% select(-Date),ntree=500, 
...
DF_1$Diff <- DF_1$Best_Pred-DF_1$median_no2
...
P <- ggplot(data=DF_1)+
  geom_point(mapping=aes(x=Date,y=median_no2,color=weekday)) +ggtitle("DIFF= PRED-OBS") + facet_wrap(weekday~.)
